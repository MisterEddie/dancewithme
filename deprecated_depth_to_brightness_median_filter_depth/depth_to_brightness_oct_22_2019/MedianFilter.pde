...
public class MedianFilter {
  ...
  */
   ...
   * cFi: the current frame's offset index in the array
   */
  public void filterFrame(int[] mLoaded, int[] mFiltrd, int[]mDepth, int cFi) {
    for (int curRow = n; curRow < mRows-n; curRow++) {
      /* Apply median filter algorithm for non-edges */
      for (int i = cFi+curRow*mCols+n; i < cFi+(curRow+1)*mCols-n; i++) {
        int iter = 0;
        // Scan the 2D surrounding box and put it into the 1D itempArr
        for (int j = -n; j <= n; j++) {
          System.arraycopy(mLoaded, i+j*mCols-n, itempArr, iter*mDim, mDim);
          iter++;
        }
        // Find the median value and set its pixel value.
        int oldVal = mFiltrd[i];
        mFiltrd[i] = findMedian(itempArr, mBox);
        
      //  // Now we have to find that index that we switched the pixel to, and move the
      //  // correct depth value in its place too. Otherwise the new pixel possibly has a
      //  // wrongly very far away depth value which messes up the brightness.
      //  if (oldVal != mFiltrd[i]) { // I think this if statement will save time, not sure though.
      //    for (int k = 0; k < mDim; k++) {
      //      if (itempArr[k] == mFiltrd[i]) {
      //        int j_rec = k/3-1; // Recover the j value corresponding to this index (which row was it)
      //        int i_rec = k % 3; // Recover the offset from j value corresponding to the index
      //        mDepth[i] = mDepth[i+j_rec*mCols-n+i_rec]; 
      //      }
      //    }
      //  }
      //}
