...
// displayText 
final String displayTextYY = "In you vs. you, we look at the places you frequent the most in space. The longer you stay in a spot, the brighter that spot is."; 
final int textPosX_YY = floor(YY_WIDTH * 0.25); 
final int textPosY_YY = floor(YY_HEIGHT * 1.1); 
final int textBoxWidth_YY = floor(YY_WIDTH * 1.5); 
...
void YY_draw() {
  ...
  for (int i = 0; i < YY_NUM_PIXELS; i++) {
    ...
    if (rawData[i] != 255) {
      ...
      color hcolor = color(heat_map.get(i)*360*1.2/YY_TOT_FRAMES, 100, 100);
      YY_img.pixels[i] = newColor;        
      YY_himg.pixels[i] = hcolor;
    } else {

      color hncolor = color(0, 0, 0); //blackout
