...
final String YA_DB_PATH = "./data/you_vs_all_base.db";
final int YA_MAX_USERS = 100; // max number of unique users in database
...
// displayText 
final String displayTextYA = "In you vs. all, as you move around, we look for previous people who were at the same place at the same time as you.";
final int textPosX_YA = floor(YA_WIDTH * 0.25); 
    ...
    for (int i = 0; i < YA_NUM_PIXELS; i++) {
      ...
      if (!pixelHasColor) {
        // fade away the background
          ...
          YA_liveImage.pixels[i] = fadedColor;
      }
      else {
...
void saveBodyDataToDatabase() {
  ...
  if (YA_db.next()) {
    ...
    if (numUsers > YA_MAX_USERS) {
      println("More than 100 past users, deleting oldest user.");
