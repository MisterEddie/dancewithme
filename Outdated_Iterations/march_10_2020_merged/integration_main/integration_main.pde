//integration_main_march_10_MERGED //<>//
import KinectPV2.*;
...
final int DISPLAY_TIME_INTRO = 2000; // 2 seconds
final int DISPLAY_TIME_TITLE = 3000; // 3 seconds 
final int DISPLAY_TIME_DESC = 6000; // 6 seconds 
...
void draw() {
  ...
  if (begin_dance) {
    ...
    // You Versus You IF SELECTEDD
    if (showIntro == false && YY_finish == false) {
      if (!YYsetUp) {
        YY_setup();
        YYsetUp = true;
      }
    ...
    // You Versus Previous if selected
    if (showIntro == false && YY_finish == true && YP_finish == false) {
      if (!YPsetUp) {
        YP_setup();
        YPsetUp = true;
      }
    ...
    // You Versus All if selected
    if (showIntro == false && YP_finish == true && YA_finish == false) {
      if (!YAsetUp) {
        YA_setup();
        YAsetUp = true;
      }
