 ...
 * General Constants 
 */
final int YP_DURATION  = 3;
final int YP_FPS       = 16;
final int YP_TOT_FRAMES = YP_DURATION * YP_FPS;
final int YP_HEIGHT = 424;
final int YP_WIDTH = 512;
final int YP_NUM_PIXELS = YP_WIDTH * YP_HEIGHT; // num pixels per inner window
final int YP_NUM_SECONDS_TO_PERSIST = 3;
final String YP_OUTPUT_FILENAME = "intersections.png";
...
void YP_setup() {
  ...
  // Set colors
  YP_PREV_PERSON_COLOR = color(YP_PREV_PERSON_HUE, 60, 80);
  YP_CURR_PERSON_COLOR = color(YP_CURR_PERSON_HUE, 60, 80);
...
void YP_draw() {
  ...
  if (YP_frameCounter == YP_TOT_FRAMES) {
    ...
    createOutputImage(YP_youAndPrevIntersectionPixels, YP_OUTPUT_FILENAME);
    createOutputImage(YP_fadedPixels, "fade.png");
  ...
  //  print("making blue...\n"); 
  //  if (YP_frameCounter == 100) {
  //    print("hello");
  //  }
  //  YP_intersectionHue = 180;  
  //}
  //else{
  //  print("making red \n"); 
  //  YP_intersectionHue = 360; 
  //}
  
  // TEMP: replace with joint similarity color
  //YP_intersectionHue = (YP_intersectionHue + 1) % 360;
  ...
  for (int i = 0; i < rawBodyData.length; i+=1){
    ...
    // No body detected at that pixel
      ...
      // the intersection shape.
        ...
        YP_youAndPrev.pixels[i] = YP_youAndPrevIntersectionPixels[i];
      }
    }
  }
  ...
  // Render images
  image(YP_justYou, 0, 0); // top left
  // This is where the heatmap stuff should go
  // image(YP_youAndPrevOutputImage, 0, 424); // bottom left
  image(YP_youAndPrev, YP_WIDTH, 0); // top middle
  image(YP_youAndPrevOutputImage, YP_WIDTH, YP_HEIGHT); // bottom middle
  image(YP_youAndPrev, YP_WIDTH*2, 0); // top right
  image(YP_youAndPrevOutputImage, YP_WIDTH*2, YP_HEIGHT); // bottom right
